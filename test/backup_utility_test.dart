...
void main() {
  ...
  Map<String, dynamic> makeSyntheticRecord({
    ...
    return {
      'id': id,
      'participant': {'studyId': studyId, 'name': name, 'indianPhone': phone},
  ...
  Future<void> populateSourceDir({
    ...
    final recordsFile = File(
      '${dir.path}${Platform.pathSeparator}$recordsFileName',
    );
    ...
    if (writeConflicts) {
      final confList = conflicts ?? [makeSyntheticConflict(id: 'conf_001')];
  ...
  group('Backup Creation', () {
    test('generates timestamped folder with records, conflicts, manifest, and SHA256SUMS', () async {
      ...
      expect(
        RegExp(
          r'^backup_\d{8}_\d{6}$',
        ).hasMatch(backupDir.uri.pathSegments.where((s) => s.isNotEmpty).last),
        isTrue,
      );
    ...
    test('manifest contains correct record counts, file sizes, and SHA-256 hashes', () async {
      final records = [
    ...
    test('missing conflicts.json in source is handled gracefully with empty conflicts list', () async {
      ...
      final verification = await verifier.verifyBackup(result.backupDirectory);
    ...
    test(
      ...
      () async {
        ...
        expect(
          () =>
              engine.backup(destination: destinationDir, source: missingSource),
          throwsA(isA<BackupValidationException>()),
        );
      },
    );

    test('rejects missing records.json in source directory with BackupValidationException', () async {
      ...
      expect(
        () => engine.backup(destination: destinationDir, source: sourceDir),
        throwsA(
    ...
    test('rejects unparseable/corrupted JSON in source records.json', () async {
      await sourceDir.create(recursive: true);
      final recFile = File(
        '${sourceDir.path}${Platform.pathSeparator}$recordsFileName',
      );
    ...
    test('rejects non-list JSON in source records.json', () async {
      await sourceDir.create(recursive: true);
      final recFile = File(
        '${sourceDir.path}${Platform.pathSeparator}$recordsFileName',
      );
    ...
    test('rejects unparseable JSON in source conflicts.json', () async {
      await populateSourceDir(dir: sourceDir, writeConflicts: false);
      final confFile = File(
        '${sourceDir.path}${Platform.pathSeparator}$conflictsFileName',
      );
    ...
    test('never overwrites existing backup folder if the same timestamp is hit', () async {
      await populateSourceDir(dir: sourceDir);
      ...
      expect(
        await canaryFile.readAsString(),
        equals('MUST_NOT_BE_OVERWRITTEN'),
      );
      ...
      expect(result.backupDirectory.path, contains('${collidingDir.path}_'));
    });
    ...
    test(
      ...
      () async {
        ...
        final folderName = result.backupDirectory.uri.pathSegments
  ...
  group('Integrity Verification', () {
    ...
    test('detects tampered content (modifying a single character in records.json fails with hash mismatch)', () async {
      await populateSourceDir(dir: sourceDir);
      final backup = await engine.backup(
        destination: destinationDir,
        source: sourceDir,
    ...
    test('detects tampered content in conflicts.json', () async {
      ...
      final result = await verifier.verifyBackup(backup.backupDirectory);
      expect(result.passed, isFalse);
      expect(result.errors.any((e) => e.contains('conflicts.json')), isTrue);
    ...
    test('detects size mismatch when extra bytes are appended', () async {
      ...
      final result = await verifier.verifyBackup(backup.backupDirectory);
      expect(result.passed, isFalse);
      expect(result.errors.any((e) => e.contains('Size mismatch')), isTrue);
    });
    ...
    test('detects tampered SHA256SUMS file', () async {
      ...
      expect(result.errors.any((e) => e.contains(sha256SumsFileName)), isTrue);
    });
  ...
  // ==========================================================================
  group('Safe Restore', () {
    test('rejects restore from tampered backup and aborts before touching target', () async {
      await populateSourceDir(dir: sourceDir);
      final backup = await engine.backup(
        destination: destinationDir,
        source: sourceDir,
      ...
      expect(
        await targetRecords.readAsString(),
        equals('CANARY_DATA_DO_NOT_TOUCH'),
      );
    });

    test('refuses to overwrite existing live target data without --confirm-overwrite', () async {
      await populateSourceDir(dir: sourceDir);
      final backup = await engine.backup(
        destination: destinationDir,
        source: sourceDir,
    ...
    test('when --confirm-overwrite is passed, creates pre-restore safety snapshot before replacing', () async {
      await populateSourceDir(dir: sourceDir);
      final backup = await engine.backup(
        destination: destinationDir,
        source: sourceDir,
      ...
      await targetRecords.writeAsString(
        jsonEncode([
          {'id': 'live_canary_rec_123'},
        ]),
      );
      await targetConflicts.writeAsString(
        jsonEncode([
          {'id': 'live_canary_conf_456'},
        ]),
      );
    ...
    test('restored content is identical to the original source', () async {
      ...
      final origConflicts = [makeSyntheticConflict(id: 'conf_exact_1')];
    ...
    test(
      'mid-write failure rolls records back and clears the marker',
      () async {
        await populateSourceDir(dir: sourceDir);
        final backup = await engine.backup(
          destination: destinationDir,
        ...
        // A directory at the second file path forces the second replacement to fail.
        await Directory(
          '${targetDir.path}${Platform.pathSeparator}$conflictsFileName',
        ).create();
        await expectLater(
        ...
        expect(await records.readAsString(), contains('original'));
        expect(
          await File(
    ...
    test('interrupted restore blocks server until verified rollback', () async {
      await populateSourceDir(dir: sourceDir);
      final backup = await engine.backup(
        destination: destinationDir,
        source: sourceDir,
      ...
      final snapshotName = restored.safetySnapshotDirectory!.uri.pathSegments
      ...
      await expectLater(
        server.LocalRecordStore(targetDir).load(),
        throwsA(isA<StateError>()),
      );
      await engine.recoverInterruptedRestore(targetDir);
      expect(await records.readAsString(), contains('before_restore'));
      expect(
        await File(
          '${targetDir.path}${Platform.pathSeparator}$conflictsFileName',
        ).exists(),
    ...
    test('corrupt safety snapshot keeps the interruption marker', () async {
      await populateSourceDir(dir: sourceDir);
      final backup = await engine.backup(
        destination: destinationDir,
        source: sourceDir,
      ...
      await targetDir.create(recursive: true);
      final records = File('${targetDir.path}${Platform.pathSeparator}$recordsFileName');
      await records.writeAsString(jsonEncode([{'id': 'original'}]));
      final restored = await engine.restore(
        backupDir: backup.backupDirectory,
        target: targetDir,
        confirmOverwrite: true,
      );
      final snapshot = restored.safetySnapshotDirectory!;
      final marker = File('${targetDir.path}${Platform.pathSeparator}$restoreMarkerFileName');
      await marker.writeAsString(jsonEncode({
        'version': 1,
        'snapshotFolder': snapshot.uri.pathSegments.where((s) => s.isNotEmpty).last,
        'hadRecords': true,
        'hadConflicts': false,
      }));
      await File('${snapshot.path}${Platform.pathSeparator}$recordsFileName')
          .writeAsString('corrupted snapshot');
      await expectLater(
        engine.recoverInterruptedRestore(targetDir),
        throwsA(isA<BackupIntegrityException>()),
      );
      expect(await marker.exists(), isTrue);
      expect(await records.readAsString(), contains('rec_001'));
    });
  });
  ...
  group('CLI execution via Process.run', () {
    test(
      ...
      () async {
        final proc = await Process.run(dartExecutable, [
          'run',
      ...
      () async {
        ...
        final proc = await Process.run(
          ...
          [
            ...
            '--label=cli-test',
            '--allow-local-test-destination',
          ],
          workingDirectory: projectRoot.path,
          environment: {'STUDY_BACKUP_TEST_MODE': '1'},
        );
    ...
    test('CLI rejects a same-PC destination for an ordinary backup', () async {
      await populateSourceDir(dir: sourceDir);
      final proc = await Process.run(dartExecutable, [
        'run',
        'tool/backup_utility.dart',
      ...
      expect(proc.stderr.toString(), contains('not verified off-PC storage'));
      expect(destinationDir.listSync(), isEmpty);
    }, timeout: const Timeout(Duration(minutes: 2)));

    test('dart run tool/backup_utility.dart verify validates backup integrity', () async {
      await populateSourceDir(dir: sourceDir);
      final backup = await engine.backup(
        destination: destinationDir,
        source: sourceDir,
      ...
      final verifyProc = await Process.run(dartExecutable, [
        'run',
        'tool/backup_utility.dart',
        'verify',
      ...
      final failProc = await Process.run(dartExecutable, [
        'run',
        'tool/backup_utility.dart',
        'verify',
        '--backup=${backup.backupDirectory.path}',
      ...
      expect(failProc.stderr.toString(), contains('hash mismatch'));
    }, timeout: const Timeout(Duration(minutes: 2)));

    test('dart run tool/backup_utility.dart restore safeguards live data and restores cleanly', () async {
      await populateSourceDir(dir: sourceDir);
      final backup = await engine.backup(
        destination: destinationDir,
        source: sourceDir,
      ...
      final blockedProc = await Process.run(dartExecutable, [
        'run',
        'tool/backup_utility.dart',
        'restore',
        '--backup=${backup.backupDirectory.path}',
      ...
      final okProc = await Process.run(dartExecutable, [
        'run',
        'tool/backup_utility.dart',
        'restore',
        '--backup=${backup.backupDirectory.path}',
      ...
      expect(okProc.stdout.toString(), contains('Pre-Restore Snapshot:'));
    ...
    test('CLI handles usage errors and prints help correctly', () async {
      // help subcommand -> exit 0 with usage
      final helpSubcmd = await Process.run(dartExecutable, [
        'run',
        'tool/backup_utility.dart',
        'help',
      ], workingDirectory: projectRoot.path);
      ...
      final helpProc = await Process.run(dartExecutable, [
        'run',
        'tool/backup_utility.dart',
        '--help',
      ], workingDirectory: projectRoot.path);
      ...
      final unknownProc = await Process.run(dartExecutable, [
        'run',
        'tool/backup_utility.dart',
        'bogus-command',
      ], workingDirectory: projectRoot.path);
      ...
      final missingDest = await Process.run(dartExecutable, [
        'run',
        'tool/backup_utility.dart',
        'backup',
      ], workingDirectory: projectRoot.path);
      ...
      final missingBackup = await Process.run(dartExecutable, [
        'run',
        'tool/backup_utility.dart',
        'verify',
      ], workingDirectory: projectRoot.path);
      ...
      final missingRestoreBackup = await Process.run(dartExecutable, [
        'run',
        'tool/backup_utility.dart',
        'restore',
      ], workingDirectory: projectRoot.path);
